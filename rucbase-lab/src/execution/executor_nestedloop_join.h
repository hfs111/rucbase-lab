...
class NestedLoopJoinExecutor : public AbstractExecutor {
   ...
   public:
    ...
    std::unique_ptr<RmRecord> Next() override {
        ...
        // memcpy()可能对你有所帮助
		auto record_l = left_->Next();
		auto record_r = right_->Next();
		memcpy(record->data, record_l->data,record_l->size);
		memcpy(record->data+record_l->size, record_r->data,record_r->size);
        // lab3 task2 Todo End
        return record;
    }
